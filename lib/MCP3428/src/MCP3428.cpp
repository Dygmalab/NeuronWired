...
*/
/***************************************************************************/
MCP3428::MCP3428(TwoWire &Wire_, uint8_t devAddress)
  : WireAdc(Wire_) {
  WireAdc.begin();
...
bool MCP3428::testConnection() {
  WireAdc.beginTransmission(devAddr);
  return (WireAdc.endTransmission() == 0);
}
...
void MCP3428::SetConfiguration(uint8_t channel, uint8_t resolution, bool mode, uint8_t PGA) {
  ...
  WireAdc.write((config |= 128));
  WireAdc.endTransmission();
}
...
bool MCP3428::CheckConversion() {
  uint8_t i   = 0;
  no_of_bytes = 3;
  WireAdc.requestFrom(devAddr, no_of_bytes);

  while (WireAdc.available()) {
    data[i++] = WireAdc.read();
